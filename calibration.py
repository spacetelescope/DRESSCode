...
def main():
    ...
    print("Converting the units of the final image from counts/s to Jy and correcting for the aperture calibration...")

    #Specify the conversion factors.
    factor_UVW2 = 8.225e-28 * 1.e23
    factor_UVM2 = 1.396e-27 * 1.e23
    factor_UVW1 = 9.524e-28 * 1.e23
   ...
   #Convert the units of the images.
    if os.path.isfile(path+"total_sum_uw2_nm.fits"):convert(path+"total_sum_uw2_nm.fits",factor_UVW2)
    if os.path.isfile(path+"total_sum_um2_nm.fits"):convert(path+"total_sum_um2_nm.fits",factor_UVM2)
    if os.path.isfile(path+"total_sum_uw1_nm.fits"):convert(path+"total_sum_uw1_nm.fits",factor_UVW1)
...
def convert(filename,factor):
    ...
    new_hdu = fits.PrimaryHDU([primary,hdulist[0].data[1],coicorr_unc,hdulist[0].data[3],poisson],header)
    new_hdu.writeto(filename.replace("total_sum",galaxy+"_final").replace("nm","Jy"),overwrite=True)
