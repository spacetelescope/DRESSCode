...
i = 0
num = sum(1 for filename in sorted(os.listdir(path)) if filename.endswith("rw.img") and not "_img_" in filename and not "_evt_" in filename or filename.endswith(".evt"))
error = False
...
for filename in sorted(os.listdir(path)):
    ...
    if not filename.endswith("rw.img") and not filename.endswith(".evt"): continue
	#Skip previously (in uvotimage.py) created raw files.
    if "_img_" in filename or "_evt_" in filename: continue
    ...
    terminal_output_file = path + "output_uvotimage_" + filename.split('_',1)[0] + "_uat.txt"
    ...
    with open(terminal_output_file,"w") as terminal:
        subprocess.call("uvotimage infile=" + infile + " prefix=" + prefix + " attfile=" + attfile + " teldeffile=CALDB alignfile=CALDB ra=" + str(RA) + " dec=" + str(DEC) + " roll=" + str(PA) + " mod8corr=yes refattopt='ANGLE_d=5,OFFSET_s=1000'", cwd=path, shell=True, stdout=terminal)
    ...
    for line in file:
        ...
        if "error" in line:
            print("An error has occured for image " + filename.rsplit('_',1)[0] + ".img")
            error = True
    ...
    #Print user information.
    i += 1
    print("Sky image created for all (other) frames of " + filename.rsplit('_',1)[0] + ".img (" + str(i) + "/" + str(num) + ")")
